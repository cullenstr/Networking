	...
	if( routing_table.ls_records[msg_source] < seq_num) //pretty sure the value will be initialized to 0 if this is the first msg
	{
		...
		while( !finished) {
			...
			if (lowest_cost==-1)
				...
				{
					if ((*col).second.cost!=-1 && (*col).second.cost+lowest_cost < routing_table.next_hop[col->first].second.cost) //if lowest cost route
					{
...
int LinkState::RecursiveBuild(int node, int next_hop, TopoLink total_cost) {
	...
	for(child_it=node_row->second.begin(); child_it!=node_row->second.end(); child_it++) {
		if(child_it->second.cost!=-1) //valid link to child node
		{
			...
			RecursiveBuild(child_it->first, next_hop, new_cost);
		}
	}
	return 0;
}
